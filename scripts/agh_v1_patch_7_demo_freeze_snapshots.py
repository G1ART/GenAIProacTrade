"""AGH v1 Patch 7 — Product-hardening freeze HTML snapshots (F2).

Captures 4 product-surface HTML snapshots + a sha256 manifest so the
Patch 7 state is reproducibly auditable. We deliberately avoid a live
browser (no playwright / selenium dependency in this repo): each
snapshot is a self-contained ``.html`` file that either mirrors the
SPA shell verbatim or embeds the canonical API payload the SPA would
render from, plus a reference to the SPA shell files' sha256 digests.

Unlike Patch 6, Patch 7's snapshots are explicitly *product hardening*
evidence (2-tier nav, Today typography, 3-cluster research, 3-lane
replay, consolidated audit), not demo-theater evidence. The freeze
verifies the exact bytes operators will see in their browser.

Files written under ``data/mvp/evidence/screenshots_patch_7/``:

    1. ``freeze_spa_index_patch_7.html``                     — SPA shell (captures the new 2-tier nav + typography CSS).
    2. ``freeze_today_object_detail_ko_patch_7.html``        — Today detail payload (KO).
    3. ``freeze_today_object_detail_en_patch_7.html``        — Today detail payload (EN).
    4. ``freeze_replay_governance_lineage_demo_patch_7.html`` — replay lineage payload.
...
EV_DIR = REPO_ROOT / "data" / "mvp" / "evidence" / "screenshots_patch_7"
...
def _render_payload_snapshot(
    *,
    ...
    body = (
        ...
        f"<div class=\"meta\">captured_at_utc={html.escape(_now_iso())} · patch=7</div>\n"
...
def main() -> int:
    ...
    # captures the exact Patch 7 DOM+CSS (2-tier nav, typography tokens,
    # audit <details> shell, 3-lane plot styles).
    src_index = STATIC_DIR / "index.html"
    snapshots.append(
        _write(
            EV_DIR / "freeze_spa_index_patch_7.html",
    ...
    snapshots.append(
        _write(
            EV_DIR / "freeze_today_object_detail_ko_patch_7.html",
            _render_payload_snapshot(
                title="Today — object detail (KO) · Patch 7 product-hardening freeze",
                payload=detail_ko,
                description=(
                    "Canonical payload of GET /api/today/object_detail?lang=ko "
                    "for the top-ranked asset. The SPA renders the Patch 7 "
                    "Today surface from this payload: hero/subhead/body "
                    "typography, recent-activity mini-list, 3-cluster "
                    "research, and the consolidated audit <details> block."
                ),
            ),
        )
    )
    ...
    snapshots.append(
        _write(
            EV_DIR / "freeze_today_object_detail_en_patch_7.html",
            _render_payload_snapshot(
                title="Today — object detail (EN) · Patch 7 product-hardening freeze",
                payload=detail_en,
                description=(
                    "Canonical payload of GET /api/today/object_detail?lang=en. "
                    "Locale coverage is still enforced by ResearchAnswerStructureV1 "
                    "guardrails; Patch 7 adds product-toned copy for the "
                    "degraded/ko_only/en_only coverage states."
                ),
            ),
        )
    )
    ...
    snapshots.append(
        _write(
            EV_DIR / "freeze_replay_governance_lineage_demo_patch_7.html",
            _render_payload_snapshot(
                title="Replay governance lineage · Patch 7 product-hardening freeze",
                payload=lineage,
                description=(
                    "Canonical payload of GET /api/replay/governance-lineage "
                    "for reg_short_demo_v0 / short horizon. The Patch 7 Replay "
                    "surface renders a 3-lane SVG timeline (apply / spectrum "
                    "refresh / sandbox followup) + step-count summary + "
                    "time-delta tooltip sub-lines from this payload."
                ),
            ),
        )
    )
    ...
    manifest = {
        "contract": "AGH_V1_PATCH_7_PRODUCT_HARDENING_SNAPSHOTS_MANIFEST_V1",
        "milestone": 18,
    ...
    print(
        f"wrote {len(snapshots)} Patch 7 product-hardening freeze snapshots + manifest to {EV_DIR}"
    )
    return 0
