...
- **`YOUR_UNIVERSE_NAME` 같은 플레이스홀더를 쓰지 말 것** — DB에 없으면 `as_of_date: null`, 심볼 0건이 된다.
- 코드·문서에서 쓰는 **표준 토큰**: `sp500_current`, `sp500_proxy_candidates_v1`, `combined_largecap_research_v1` (`research.universe_slices`).
- **`combined_largecap_research_v1`** 은 멤버십 테이블에 행이 없을 수 있다(런타임 합집합). 커버리지 측정은 보통 **`sp500_current`**.
- **본인 DB에 실제로 무엇이 있는지**: `python3 src/main.py list-universe-names` → JSON 필드 **`use_for_phase17_cli`** 에 나온 문자열을 그대로 `--universe`에 복사.
...
python3 src/main.py run-public-depth-expansion --universe sp500_current
