...
SHELL: dict[str, dict[str, str]] = {
    "ko": {
        ...
        # AGH v1 Patch 9 B4 — self-serve 표현의 원칙:
        # 1) 실행 책임은 워커에게 있음 (사용자의 "바로 실행" 오해 방지).
        # 2) 승격·레지스트리 변경은 별도 승인 단계 (자동 승격 없음).
        # 3) UI 실행 비활성 상태에서는 복붙 경로가 1급 시민.
        "research_section.invoke_copy_hint": "터미널에서 아래 명령을 복붙해 실행하세요. UI에서 바로 실행하지 않습니다 (운영자 게이트).",
        "research_section.invoke_ui_hint": "대기열에 적재되었습니다. 다음 harness-tick 사이클에서 워커가 집어 갑니다. 승격·레지스트리 변경은 별도 승인 절차입니다.",
        "research_section.invoke_copy_btn": "복사",
        "research_section.invoke_copy_done": "복사됨.",
        "research_section.invoke_enqueue_btn": "대기열에 적재 (운영자 게이트, 자동 승격 없음)",
        ...
        # AGH v1 Patch 9 B2 — in-flight 상태용 워커 틱 힌트.
        # 샌드박스 워커는 harness-tick --loop 프로세스가 주기적으로 실행합니다.
        # 이 문구는 "자동 실행됐다" 는 오해를 피하기 위한 정직한 부가 설명입니다.
        "research_section.invoke_worker_tick_hint": "워커 틱이 30초 주기로 돌아갑니다. 운영 환경에서도 백그라운드 자동 실행은 없습니다.",
        ...
        "research_section.recent_requests_empty": "아직 요청이 없습니다. 이 지평에 대한 첫 요청을 대기열에 적재하세요.",
        # AGH v1 Patch 9 B1 — product-grade drawer for each recent request.
        # UI is intentionally honest: the "next step" copy never claims the
        # system has already acted; it restates the worker tick / approval
        # structure at the point the operator is looking at it.
        "research_section.recent_request_kind_head": "요청 종류",
        "research_section.recent_request_kind_value": "샌드박스 종류: {kind}",
        "research_section.recent_request_input_head": "요청 입력",
        "research_section.recent_request_input_horizon": "지평: {horizon}",
        "research_section.recent_request_result_head": "결과 요약",
        "research_section.recent_request_result_empty": "결과 레퍼런스가 비어 있습니다.",
        "research_section.recent_request_blocking_head": "차단 이유",
        "research_section.recent_request_next_head": "다음 단계",
        "research_section.recent_request_next_queued": "대기열에 적재된 상태입니다. 다음 harness-tick 에서 워커가 집어 갑니다.",
        "research_section.recent_request_next_running": "워커가 처리 중입니다. 완료 시 결과 레퍼런스가 기록되고, 승격은 별도 운영자 승인이 필요합니다.",
        "research_section.recent_request_next_completed": "완료되었습니다. Replay 에서 lineage 를 확인하거나, 승격은 별도 승인 절차를 거칩니다.",
        "research_section.recent_request_next_blocked": "차단되었습니다. 위의 차단 이유를 해결한 뒤 동일 요청을 다시 적재하세요.",
        ...
        # AGH v1 Patch 9 A1/D1 — v2 번들이 있으나 무결성 게이트에 실패해 v0로 폴백한 상태를 정직하게 표시
        "tsr.bundle_tier.fallback": "번들: 폴백 (v0)",
        "tsr.bundle_tier.fallback_tip": "프로덕션 v2 번들이 무결성 게이트에 실패해 v0 샘플 번들로 폴백되었습니다. /api/runtime/health 의 degraded_reasons 에서 상세를 확인하세요.",
        ...
        "tsr.invoke.contract.will_do": "샌드박스 큐에 bounded validation_rerun 요청 1건을 적재합니다.",
        "tsr.invoke.contract.will_not_do": "활성 레지스트리 변경·아티팩트 승격·Today 화면 갱신은 하지 않습니다.",
        "tsr.invoke.contract.after_enqueue": "워커가 `harness-tick --queue sandbox_queue` 주기에서 이 요청을 집어 갑니다.",
        # AGH v1 Patch 8 B4 — contract card 4번째 줄: 실행 후 관찰 가능한 상태
        "tsr.invoke.contract.status_after": "아래 4-state 칩(queued / running / completed / blocked)이 실시간 상태를 보여 줍니다.",
        # AGH v1 Patch 9 B3 — 2-column grid cell heads.
        "tsr.invoke.contract.cell_head.will_do": "수행",
        "tsr.invoke.contract.cell_head.will_not_do": "수행하지 않음",
        "tsr.invoke.contract.cell_head.after_enqueue": "적재 후",
        "tsr.invoke.contract.cell_head.status_after": "실행 후",
        "tsr.nav.primary.aria": "주요 작업",
        "tsr.nav.utility.aria": "보조 작업",
        "tsr.nav.utility.note": "운영 유틸 · 감사/원본 데이터/번들 재로드",
    ...
    "en": {
        ...
        # AGH v1 Patch 9 B4 — self-serve copy hardening. No phrasing may
        # claim auto-execution, auto-promotion, or UI-side compute.
        "research_section.invoke_copy_hint": "Copy and run the command below in a terminal. The UI does not run it directly (operator gate).",
        "research_section.invoke_ui_hint": "Enqueued. The worker picks this up on the next harness-tick cycle. Promotion / registry changes stay on the separate approval path.",
        "research_section.invoke_copy_btn": "Copy",
        "research_section.invoke_copy_done": "Copied.",
        "research_section.invoke_enqueue_btn": "Enqueue (operator-gated; no auto-promotion)",
        ...
        # AGH v1 Patch 9 B2 — in-flight worker-tick hint.
        "research_section.invoke_worker_tick_hint": "Worker ticks run on a 30s cadence. Even in production there is no hidden background executor.",
        ...
        "research_section.recent_requests_empty": "No requests yet. Enqueue the first request for this horizon.",
        # AGH v1 Patch 9 B1 — product-grade drawer for each recent request.
        "research_section.recent_request_kind_head": "Request kind",
        "research_section.recent_request_kind_value": "Sandbox kind: {kind}",
        "research_section.recent_request_input_head": "Request input",
        "research_section.recent_request_input_horizon": "Horizon: {horizon}",
        "research_section.recent_request_result_head": "Result summary",
        "research_section.recent_request_result_empty": "Result payload carried no refs.",
        "research_section.recent_request_blocking_head": "Blocking reasons",
        "research_section.recent_request_next_head": "Next step",
        "research_section.recent_request_next_queued": "Enqueued. The next harness-tick worker run will pick this up.",
        "research_section.recent_request_next_running": "Worker is processing. When it finishes, result refs are recorded; promotion still needs separate operator approval.",
        "research_section.recent_request_next_completed": "Completed. Inspect lineage in Replay. Promotion is a separate approval step.",
        "research_section.recent_request_next_blocked": "Blocked. Resolve the reasons above, then enqueue the same request again.",
        ...
        # AGH v1 Patch 9 A1/D1 — honest label for "v2 present but failed the quick integrity gate; v0 served"
        "tsr.bundle_tier.fallback": "Bundle: fallback (v0)",
        "tsr.bundle_tier.fallback_tip": "Production v2 bundle failed the quick integrity gate; served v0 sample bundle instead. See degraded_reasons in /api/runtime/health.",
        ...
        "tsr.invoke.contract.will_do": "Enqueue one bounded validation_rerun request onto the sandbox queue.",
        "tsr.invoke.contract.will_not_do": "Does not modify the active registry, promote artifacts, or refresh Today.",
        "tsr.invoke.contract.after_enqueue": "The worker picks this up on the next `harness-tick --queue sandbox_queue` cycle.",
        "tsr.invoke.contract.status_after": "The 4-state chip below (queued / running / completed / blocked) reflects the live state.",
        # AGH v1 Patch 9 B3 — 2-column grid cell heads.
        "tsr.invoke.contract.cell_head.will_do": "Will do",
        "tsr.invoke.contract.cell_head.will_not_do": "Will NOT do",
        "tsr.invoke.contract.cell_head.after_enqueue": "After enqueue",
        "tsr.invoke.contract.cell_head.status_after": "After run",
        "tsr.nav.primary.aria": "Primary actions",
        "tsr.nav.utility.aria": "Utility actions",
        "tsr.nav.utility.note": "Ops utilities · audit, raw data, bundle reload",
