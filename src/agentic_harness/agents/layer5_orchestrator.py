...
_SYSTEM_PROMPT = (
    ...
    "what_changed_bullets_ko / what_changed_bullets_en / "
    "residual_uncertainty_bullets / what_to_watch_bullets; each <= 280 "
    "chars, each list <= 6 items) and an evidence_cited array that is a "
    # AGH v1 Patch 8 A1b — what_changed_bullets are a distinct 'what is new '
    # 'since the user last saw this' layer. Do NOT duplicate summary bullets "
    "there: ``summary_bullets_*`` answers 'why this matters', "
    "``what_changed_bullets_*`` answers 'what is genuinely new'. Leave "
    "what_changed_bullets empty rather than padding when nothing has "
    "materially changed since the prior view. "
