...
def merge_phase52_into_summary(summary: dict[str, Any], repo_root: Path) -> None:
    reg_p = default_external_source_registry_path(repo_root)
    if not reg_p.is_file():
        return
    reg = load_source_registry(reg_p)
    if not (reg.get("sources") or []):
        return
    bud = load_budget_state(default_budget_state_path(repo_root))
    qd = queue_depth_pending(default_event_queue_path(repo_root))
