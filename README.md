...
- **Private Beta — Invite-Only Auth + Account-Level Usage Tracking (Patch 12, 2026-04-23)**: Patch 11 이 봉인한 Brain/Product 계약은 한 글자도 건드리지 않고 **제품 외부 경계 (인증/세션/이벤트/배포/RLS)** 만 추가한 온라인 전환 패치입니다. (1) **DB**: `supabase/migrations/20260423100000_patch_12_private_beta_auth_tracking_v1.sql` 가 `beta_users_v1` (초대/활성/보류/철회 상태머신 + `beta_user/admin/internal` 역할) + `profiles_v1` (자기 row SELECT/UPDATE only) + `product_usage_events_v1` (service_role only, `occurred_at/user_id/session_id/event_name/surface/route/asset_id/horizon_key/result_state/lang/metadata` 11 칼럼 + 3 인덱스 + 4 admin view (`v_beta_users_active_v1`, `v_beta_sessions_recent_v1`, `v_beta_top_events_v1`, `v_beta_trust_signals_v1`)) 를 RLS 정책과 함께 정의. (2) **서버 인증 스택**: `src/phase47_runtime/auth/` 는 **stdlib-only HS256 JWT verifier** (`hmac + hashlib + base64 + json`, 외부 의존 0), `beta_allowlist.py` (`enforce/shadow/off` 3 모드 + TTL 캐시), `guard.py` 의 `require_auth` (공개 path `/api/runtime/health` · `/api/runtime/auth-config` · `/api/auth/session` · `/api/auth/signout` 4개, OPTIONS 프리플라이트 바이패스, **secret 미설정 시 graceful downgrade** 로 로컬/CI 무중단), `supabase_rest.py` (stdlib `urllib.request` 기반 최소 PostgREST client, `supabase-py` 의존 0) 로 구성. `routes.py` 최상단에 가드가 박혀 모든 `/api/*` 가 동일 JWT 체크를 받는다. (3) **Auth 라우트 4 개**: `GET /api/runtime/auth-config` (브라우저용 공개 anon_key + URL), `POST /api/auth/session` (JWT verify → `beta_users_v1` 상태 체크 → `profiles_v1` upsert + `last_seen_at` 갱신 → 12자 해시 `user_id_alias` 반환), `GET /api/auth/me`, `POST /api/auth/signout`. (4) **Telemetry 인제스트**: `telemetry/event_taxonomy.py` 가 **13-event allowlist** (`session_started / page_view / research_opened / replay_opened / ask_opened / ask_quick_action_clicked / ask_free_text_submitted / ask_answer_rendered / ask_degraded_shown / sandbox_enqueue_clicked / sandbox_request_blocked / out_of_scope_shown / hero_card_focused`) + `ALLOWED_SURFACES` / `ALLOWED_HORIZON_KEYS` / `ALLOWED_LANGS` / `MAX_METADATA_BYTES` (2KB) 를 고정. `telemetry/ingest.py` 가 `strip_engineering_ids` 재사용 + UUID 세션 shape 검사 + **in-memory sliding-window rate limit (100/min/user)** + service_role insert 를 수행. `POST /api/events` (단건) + `POST /api/events/batch` (최대 50, **partial-save 금지** — audit-friendly). (5) **/ops Beta Admin 탭**: 기존 `/ops` (env-gated, customer 엔 404 유지) 의 `METIS_OPS_SHELL=1` 표면에 "Beta Admin" 탭 1개 추가, `ops_admin.js` 가 4 섹션 (Invited users / Recent sessions 24h / Top events 7d / Trust signals rate) 를 SVG hand-roll 바차트로 렌더. `/api/admin/beta/{users,sessions,events,trust}` 는 `admin`/`internal` role 만 통과 (beta_user → 403). 모든 admin DTO 에서 raw UUID 는 `bu_<sha256[:12]>` 로 aliased, 이메일은 서버 측 마스킹. (6) **Login UI + SPA bootstrap**: `static/login.html` + `login.css` + `login.js` (supabase-js ESM CDN import, `signInWithOtp` + hash `#access_token` 감지 후 `POST /api/auth/session`), `auth_bootstrap.js` 가 Product Shell 첫 렌더 전 세션 체크 → 없으면 `/login.html` 리다이렉트, 있으면 **`window.fetch` 를 글로벌 래핑** 해 모든 `/api/*` 에 Bearer JWT 주입 + `window.metisEmitEvent(name, meta)` 전역 헬퍼 발행. `product_shell.js` 의 panel switch / Ask quick-chip / Ask free-text / degraded 경로에 이벤트 훅이 얹혀 있다. (7) **Config + env**: `src/config.py` 에 `SUPABASE_ANON_KEY`, `SUPABASE_JWT_SECRET`, `SUPABASE_AUTH_REDIRECT_URL`, `METIS_BETA_ALLOWLIST_MODE`, `METIS_TELEMETRY_ENABLED` 5 개 (모두 optional, 미설정 시 graceful), `.env.example` 에 주석 + 샘플값. (8) **PII 디시플린**: raw `user_id` UUID 는 클라이언트/로그/DTO 에 0회 노출, 오직 `bu_<12hex>` alias 만 순환; 이메일은 `beta_users_v1` 외부엔 등장 안 함; JWT secret 은 서버 env 에만 존재, `/login.html` + 정적 자산 어디에도 substring 0. (9) **테스트 + freeze + runbook + evidence**: `src/tests/test_agh_v1_patch_12_{jwt_verify,auth_guard,beta_allowlist,event_taxonomy,telemetry_ingest,admin_surface,copy_no_leak}.py` 7 파일 × 62 케이스 all green, `scripts/agh_v1_patch_12_beta_freeze.py` 가 `/login.html` + login/css/js + auth DTO 3 + admin DTO 4 + SHA256 manifest 를 `data/mvp/evidence/screenshots_patch_12/` 에 기록, `scripts/agh_v1_patch_12_private_beta_runbook.py` 가 S1..S7 (valid auth / revoked user / valid+invalid event ingest / rate limit / admin RBAC / no-leak) 전부 `all_ok=true`, `scripts/agh_v1_patch_12_extra_evidence.py` 가 5 건의 카테고리 evidence 를 생성, 총 `patch_12_{auth_flow,beta_allowlist,event_taxonomy,telemetry_ingest,admin_surface,private_beta_runbook}_evidence.json` 6 건 적재. (10) **운영 문서**: `docs/ops/METIS_Private_Beta_Deployment_Runbook_v1.md` (Supabase 마이그레이션 → Auth 설정 → Railway env 5키 → 첫 invite 까지 **copy-paste 가능한** 9 단계 + smoke test + rollback), `docs/ops/METIS_Beta_Invite_Checklist_v1.md` (invite/activate/pause/revoke SQL + 확인 쿼리 + admin UI 일치 체크). Brain 계약 변경 0 / Patch 11 의 Q1..Q12 spec survey 불변 / Ops Cockpit 은 여전히 customer 엔 404 / operator gate + `SANDBOX_KINDS=('validation_rerun',)` + Vanilla JS SPA + LLM 직접 쓰기 금지 전부 유지. 상세: [`docs/ops/METIS_Private_Beta_Deployment_Runbook_v1.md`](docs/ops/METIS_Private_Beta_Deployment_Runbook_v1.md).
- **Brain Bundle v3 / Signal Quality / Long-Horizon Truth (Patch 11, 2026-04-23)**: 10A/10B/10C 가 닫은 네 고객 표면의 제품 언어는 그대로 두고, 그 아래 **Brain bundle 계약** 을 Brain layer 에서 봉인한 기술 패치입니다. (1) **Residual semantics → Product shell 실전 배선** — `view_models_common.RESIDUAL_WORDING` (KO/EN × recheck·invalidation 축, 11 버킷) + `residual_freshness_block` 가 raw slug (`spectrum_position_crosses_midline` 등) 을 **한 글자도 유출 없이** 제품 라벨로 번역, `build_shared_focus_block` 이 네 표면에 embed, `compute_coherence_signature` 가 residual cadence/invalidation 을 12-hex 지문에 흡수. (2) **Long-horizon evidence honest tier** — 신규 `src/metis_brain/long_horizon_evidence_v1.py` 가 `LongHorizonSupportV1` + `classify_long_horizon_tier` (production ≥0.8&≥30 / limited ≥0.4&≥10 / sample) + `long_horizon_support_integrity_errors` 를 제공, `BrainBundleV0.long_horizon_support_by_horizon` 으로 흐르고 `validate_active_registry_integrity` 가 "horizon_provenance=real_derived 인데 tier=sample" 같은 과장을 차단. (3) **Brain overlay 공유 focus 노출** — `view_models_common.overlay_note_block` 이 `bundle.brain_overlays` 에서 focus horizon 해당 overlay 를 우선순위로 뽑아 `{dominant_kind_key, counter_interpretation_present, note_{ko,en}}` 로 요약, Today hero (via shared_focus) / Research counter_or_companion / Ask AI why_confidence 가 동일 note 를 consume. `ovr_*` / `pcp_*` / `persona_candidate_id` / `overlay_id` / `brain_overlay_ids` 는 `strip_engineering_ids` regex 5 개로 pin 돼 DTO/UI 노출 0. (4) **Q11/Q12 자동 spec survey 확장** — `mvp_spec_survey_v0.build_mvp_spec_survey_v0` 에 Q11 (`signal_quality_accumulation_ok` — short/medium residual 커버리지 ≥ 0.8) + Q12 (`long_horizon_honest_tier_ok` — provenance/tier 일관성 + integrity errors = 0) 를 **추가**, Q1..Q10 의 question_id/순서/판단 로직 **불변**. (5) **Ask AI 의미적 품질 회귀** — `data/mvp/ask_semantic_golden_set_v1.json` 18 엔트리 + `test_agh_v1_patch_11_ask_semantic_quality.py` 가 `score = 0.4*grounded + 0.3*bounded + 0.3*useful` 로 `regression_score >= 0.75` + `bounded_strict=1.0` (claim/evidence 에 매수·매도·가격 목표가 0 글자) 을 강제, `scripts/agh_v1_patch_11_ask_semantic_regression.py` 는 오퍼레이터 러너. (6) **Evidence + Runbook** — `patch_11_brain_bundle_v3_evidence.json`, `patch_11_long_horizon_truth_evidence.json`, `patch_11_signal_quality_accumulation_evidence.json`, `patch_11_ask_semantic_golden_set_evidence.json`, `patch_11_brain_truth_runbook_evidence.json`, `patch_11_brain_truth_bridge_evidence.json` 6 건 + `scripts/agh_v1_patch_11_brain_truth_freeze.py` (12 DTO + manifest, all_ok=true) + `scripts/agh_v1_patch_11_brain_truth_runbook.py` (S1..S7 전부 green). 안전 가드 전부 유지 (LLM 직접 쓰기 없음, 새 sandbox kind 없음, operator gate 유지, Vanilla JS SPA 유지). 상세: [`docs/plan/METIS_Product_Shell_Rebuild_v1_Spec_KR.md` §12](docs/plan/METIS_Product_Shell_Rebuild_v1_Spec_KR.md) + [`docs/plan/METIS_Residual_Score_Semantics_v1.md`](docs/plan/METIS_Residual_Score_Semantics_v1.md).
- **Product Shell Coherence / Trust Closure (Patch 10C, 2026-04-23)**: 10A·10B 에서 펼쳐둔 네 고객 표면 (Today / Research / Replay / Ask AI) 이 **같은 focus 에 대해 같은 진실을 같은 제품 언어로 말한다** 를 코드 수준 불변식으로 봉인한 seal patch 입니다. (1) **coherence signature** — `view_models_common.py` 의 `compute_coherence_signature` 가 `(asset_id, horizon_key, 양자화된 position, grade, stance, source, digest(what_changed), digest(rationale))` 로부터 언어 독립적 12-hex SHA-256 지문을 만들고, `build_shared_focus_block` 이 네 DTO 모두에 **문자 단위 동일한 focus 블록**을 embed 합니다. 같은 focus 는 Today / Research / Replay / Ask 어디에서 봐도 지문이 같고, 언어만 바꾸면 지문은 그대로 유지됩니다. (2) **Ask AI 3중 grounding guard** — pre-LLM `classify_question_scope` 가 매수/매도 권유·price target·화면에 없는 종목·범위 밖 주제를 LLM 호출 **없이** out-of-scope 로 차단; `surfaced_context_summary` 가 화면에 실제로 노출된 focus 를 scrubbed 한 문단으로 LLM 프롬프트에 주입; post-LLM `scan_response_for_hallucinations` 가 advice / foreign ticker / price target 을 검출하면 body 를 **폐기** 하고 partial 답변으로 downgrade. (3) **focus continuity UI** — Research/Replay/Ask 공통 `.ps-focus-ribbon` 이 같은 grade/stance/confidence chip + 네 표면 soft-link + coherence 지문을 보여주고, Today hero 아래에는 `cta_more` soft-link chip 으로 Replay·Ask AI 로 포커스 유지한 채 이동. (4) **language contract** — `SHARED_WORDING` 10 버킷 (sample / preparing / limited_evidence / production / freshness / bounded_ask / next_step / what_changed / knowable_then / out_of_scope) + `product_shell.continuity.*`, `product_shell.trust.*`, `product_shell.ask.out_of_scope.*` 세 locale 패밀리 KO/EN parity. (5) **evidence** — freeze 스크립트가 동일 focus 의 6 DTO × 2 언어 = 12 파일 + SHA256 manifest 를 `data/mvp/evidence/screenshots_patch_10c/` 에 기록하고, runbook 이 S1..S6 시나리오 (cross-surface / trust closure / continuity UI / DTO refinement / language / no-leak+fingerprint) 을 전부 green 으로 통과. 상세: [`docs/plan/METIS_Product_Shell_Rebuild_v1_Spec_KR.md` §11](docs/plan/METIS_Product_Shell_Rebuild_v1_Spec_KR.md).
- **Product Shell Rebuild — Research / Replay / Ask AI 정식 재설계 (Patch 10B, 2026-04-23)**: 10A 에서 분리한 `/` Product Shell 에 **세 표면을 제품 언어로 올렸습니다**. (1) **Research** — `GET /api/product/research?presentation=landing` 가 horizon 4 컬럼 × top-N 타일 (각 타일에 grade/stance/confidence + 한 줄 요약 + deep-link) 을 돌려주고, `presentation=deepdive&asset_id=&horizon_key=` 가 claim / 5 evidence cards (`what_changed`, `strongest_support`, `counter_or_companion`, `missing_or_preparing`, `peer_context`) / 3 action chips 의 3-rail 디프다이브를 반환. Today 내부 evidence drawer 로 답을 주던 10A 의 계약을 **누출 없이 종목 전체 답으로 확장** (soft-link, 브라우저 이동). (2) **Replay** — `GET /api/product/replay?asset_id=&horizon_key=` 가 `api_governance_lineage_for_registry_entry` 의 governance chain + sandbox followups 를 **타임라인 이벤트 + 30일+ 공백 주석 + 시작/최근 체크포인트 + baseline / weakened / stressed 3 시나리오 + advanced_disclosure 힌트 (ops 만 가리킴, payload 누출 0)** 로 번역. (3) **Ask AI** — 4 라우트 (`GET /api/product/ask`, `/api/product/ask/quick`, `POST /api/product/ask`, `GET /api/product/requests`) 가 bounded decision assistant 표면 제공: focus context card (보고 있는 종목/구간/grade/stance/confidence 한 장), 6 quick-action chips (`explain_claim, show_support, show_counter, other_horizons, why_confidence, whats_missing`, 모두 **노출된 근거 안에서만** 결정론적으로 생성), retrieval-grounded 자유 입력 (`api_conversation` 래퍼, 실패·빈 응답 시 `_degraded_answer` 구조로 자동 전환), 진행 중/완료/차단 3 상태 request-state 카드. 프론트엔드는 `STATE.focus = {asset_id, horizon_key}` + URL hash routing (`#research?asset=AAPL&h=short` 왕복 동기화 + 브라우저 back/forward) + Today hero secondary CTA 활성화 + selected_movers "자세히 보기 →" soft-link 로 **표면 간 연결성** 을 실현. 모든 DTO 는 `strip_engineering_ids` (10B 에서 `job_*`, `sandbox_request_id`, `process_governed_prompt`, `counterfactual_preview_v1`, `sandbox_queue` 금지 패턴 추가) 를 마지막 방어선으로 통과. 로캘은 `product_shell.research.*` 21 쌍 + `product_shell.replay.*` 17 쌍 + `product_shell.ask.*` 24 쌍 (KO/EN parity 62 쌍 추가) + `test_agh_v1_patch_10b_copy_no_leak.py` 118 파라미터 green. Freeze `scripts/agh_v1_patch_10b_product_shell_freeze_snapshots.py` (DTO 샘플 14 + Shell HTML/JS/CSS + SHA256 manifest), Runbook `scripts/agh_v1_patch_10b_product_shell_runbook.py` (S1..S7 × `all_ok`). 상세: [`docs/plan/METIS_Product_Shell_Rebuild_v1_Spec_KR.md`](docs/plan/METIS_Product_Shell_Rebuild_v1_Spec_KR.md).
- **사용자용 Product Shell 분리 (Patch 10A, 2026-04-23)**: 기존 Phase 47 Cockpit 을 `static/ops.html` + `static/ops.js` 로 격리하고 (**`/ops` 경로는 `METIS_OPS_SHELL=1` 일 때만 노출, 기본은 404**), 신규 `static/index.html` + `product_shell.js` + `product_shell.css` 가 **사용자용 Product Shell** 을 구성합니다. `/api/product/today` 엔드포인트가 `src/phase47_runtime/product_shell/view_models.py` 의 mapper 레이어를 거쳐 **엔지니어링 ID (`art_*`, `reg_*`, `factor_*`, `horizon_provenance` 등) 를 한 번 더 스크러빙**한 `PRODUCT_TODAY_V1` DTO 만 반환합니다. Today 한 장은 **trust strip → today-at-a-glance → hero horizon cards ×4 (grade + stance + confidence 분리 병치, SVG hand-roll 스파크라인, inline evidence drawer) → selected movers → watchlist strip (subdued) → advanced disclosure** 레이아웃으로, 샘플 데이터일 때는 "샘플 / 준비 중" 이라고 **정직하게 말합니다**. Research / Replay / Ask AI 는 10B 에서 정식 재설계되며 10A 에서는 스텁 안내 카드로 남아 있습니다. 상세: [`docs/plan/METIS_Product_Shell_Rebuild_v1_Spec_KR.md`](docs/plan/METIS_Product_Shell_Rebuild_v1_Spec_KR.md), 운영 절차: [`docs/ops/METIS_Product_Shell_vs_Ops_Cockpit_Split_Runbook_v1.md`](docs/ops/METIS_Product_Shell_vs_Ops_Cockpit_Split_Runbook_v1.md).
...
- **Agentic Operating Harness v1 (Stage 2.5)**: Patch 1 (live ingest adapter + retry/backoff), Patch 2 (RegistryUpdateProposalV1 → RegistryDecisionPacketV1 → RegistryPatchAppliedPacketV1 승격 다리), Patch 3 (artifact promotion → spectrum refresh → Today reflection), Patch 4 (validation → governance evaluator가 challenger-ready `ValidationPromotionEvaluationV1` + `RegistryUpdateProposalV1` 자동 방출), Patch 5 (Research Ask / bounded sandbox closure — `layer5_intent_router_v1` + `ResearchAnswerStructureV1` + `SandboxRequestPacketV1` / `SandboxResultPacketV1` + `sandbox_queue`), Patch 6 (Demo Freeze / Product Surface / Live Evidence Closure — Today 4-block 렌더러 + Research 5-section + Replay governance lineage compact + SVG timeline + `locale_coverage` guardrail + `POST /api/sandbox/enqueue` operator-gated), Patch 7 — Product hardening / UX depth / scale-readiness (2-tier nav, Today typography/audit consolidation, Research 3-cluster, Replay 3-lane timeline, tooltip multi-part split, bounded action contract card, governance_scan N+1 hoist, rows_limit / governance-lineage limit, perf_counter JSON logs), **Patch 9 — Productionize / Self-Serve / Scale Closure**: (1) **A1 `brain_bundle_path()` env>v2>v0 auto-detect** + quick integrity gate + `/api/runtime/health.mvp_brain_gate.brain_bundle_{path_resolved,v2_exists,v2_integrity_failed,fallback_to_v0}` + **UI fallback chip variant** (`tsr-chip--degraded`) + v2 integrity fail 시 `degraded_reasons.append("v2_integrity_failed")` (조용히 덮지 않음), (2) **A2 production-tier 4 integrity checks** (`validate_active_registry_integrity(..., tier="production")` = active/challenger / spectrum rows per horizon / tier metadata / write evidence) + `BrainBundleV0.metadata` 필드, (3) **A3 `METIS_Production_Bundle_Graduation_Runbook_v1.md`** (pre-flight → build+verify → rollback → 실패모드표 → sign-off), (4) **B1 recent sandbox requests 드로어** (`renderRecentSandboxRequestRow` + humanized summary + lifecycle-aware next step hint + premium empty state), (5) **B2 워커 tick hint** (queued/running 에서만 표시, 자동 승격 환상 제거), (6) **B3 contract card 2×2 grid** (`.tsr-contract-grid`), (7) **B4 invoke copy hardening** (운영자/operator + 대기열/queue + "자동 승격 없음" 명시), (8) **C·A 패킷/잡 retention archive** (`agentic_harness_packets_v1_archive` + `agentic_harness_queue_jobs_v1_archive` + `public.agentic_harness_count_packets_by_layer_v1()` RPC + `src/agentic_harness/retention/archive_v1.py` + `harness-retention-archive` CLI, copy-then-delete + dry-run + terminal-only jobs + bounded batch), (9) **C·B `target_scope` JSONB 인덱스** (`target_scope->>'asset_id'` + `->>'horizon'` b-tree) + `list_packets(target_asset_id, target_horizon)` push-down + layer5 orchestrator wiring, (10) **C·C message snapshot lazy generation** (`build_today_spectrum_payload` 에서 제거, `build_today_object_detail_payload` 진입 시 `persist_message_snapshot_for_spectrum_row` 로 1 건만), (11) **D1 bundle-tier chip fallback variant** + `tsr.bundle_tier.fallback_tip` ("degraded_reasons" 키워드 요구), (12) **D2 primary nav 강조 / utility demote (feature 제거 0)**, (13) **D3 no-leak 스캐너 Patch 9 확장** + real-user wording 파라미터 테스트. **Patch 9 는 demo-theater 패치가 아니라 v2 production bundle 의 런타임 기본 경로 격상 + bounded self-serve 제품화 + Scale Note Patch 8 §4.3 의 retention/JSONB/snapshot 세 건을 실코드로 닫은 것**. 안전 가드는 모두 유지 (LLM 직접 쓰기 없음, `SANDBOX_KINDS=('validation_rerun',)` 유지, operator gate 유지, fake autonomy 금지, Vanilla JS SPA 유지). **Patch 8 — Production graduation / UX-AI wow / Scale closure**: (1) **A1 Research 4-stack** (`what_changed_bullets_ko/en` 이 `ResearchAnswerStructureV1` 계약 + guardrail + orchestrator prompt 에 등장, UI 는 "What changed" + "Why it matters" 2 헤더로 재배치), (2) **A2 Today 히어로 4-row stack** (Why now / Confidence band / Caveat / Next step + jump-to-invoke), (3) **A3 Replay lineage step note + 30-day+ gap annotation**, (4) **A4+D1 로캘 `demo` → `sample` graduation** (`LEGACY_LOCALE_ALIASES` 유예) + no-leak 테스트, (5) **A5 tooltip `sub` 에 what_changed/confidence 합성**, (6) **B2 샌드박스 큐 4-state** (`queued/running/completed/blocked`) + `humanizeProducedRefs`, (7) **B3 per-entry recent sandbox requests** compact list (최근 5건, auto-refresh), (8) **B4 contract card status_after + state slot** (4-state chip mirror), (9) **C1 factor validation 배치 upsert** (`upsert_factor_validation_summaries/.._quantile_results/.._coverage_reports`) + `panel_truncated_at_limit` 가시화, (10) **C2 번들 panel cache + evaluator `reload_between_specs`** single-reload, (11) **D2 `v2` production bundle graduation script** (`agh_v1_patch_8_production_bundle_graduation.py` + atomic write + SHA256 evidence), (12) **D3 3-tier vocabulary `demo/sample/production`** + `/api/runtime/health.mvp_brain_gate.brain_bundle_tier` + UI `#tsr-bundle-tier` chip + `METIS_Production_Bundle_Graduation_Note_v1.md`, (13) **E1 `harness-tick --queue <name>` + `--loop --sleep` 워커 모드**, (14) **E2 `/api/runtime/health` degraded 200 + `degraded_reasons`** (down 만 503), (15) **E3 Railway/Supabase 배포 런북** (`docs/ops/METIS_Railway_Supabase_Deployment_Runbook_v1.md`), (16) **E4 `Procfile` + `railway.json` + `.env.example`** (web + worker). **Patch 8 은 demo-theater 패치가 아니라 실제 프로덕션 graduation + AI 유용성 wow + 500 티커 병목 상위 2건 실해소 + Railway 원클릭 배포**. 안전 가드는 모두 유지 (LLM 직접 쓰기 없음, `SANDBOX_KINDS=('validation_rerun',)` 유지, operator gate 유지, fake autonomy 금지).
- **아직 데모에 가까운 것(실질)**: `data/mvp/metis_brain_bundle_v0.json`은 여전히 시드 스펙트럼과 맞춘 **`sample` 번들**입니다. `v2` (`data/mvp/metis_brain_bundle_v2.json`) 는 Patch 8 의 D2 graduation script + Patch 9 의 A2 `tier="production"` integrity 4 체크 (active/challenger 일관성 / horizon 별 spectrum rows ≥ 1 / tier metadata 가 `pit:demo:*` / `stub_feature_set` / `deterministic_kernel` 로 시작하지 않음 / write evidence) 를 함께 통과해야 비로소 `production` tier 로 표기됩니다. Patch 9 Scale Readiness Note (`docs/plan/METIS_Scale_Readiness_Note_Patch9_v1.md`) 는 CF-8·A retention / CF-8·B JSONB 인덱스 / CF-8·C message snapshot lazy-gen 세 건이 **실코드로 닫힌 상태** 를 기록하고 남은 blocker (CF-8·D horizon_lens cache / CF-9·A retention 주기화 / CF-9·B r-branch 자동화) 를 Patch 10 범위로 이월. 500-티커 **operational green verdict (Patch 9)**: (a) v2 번들이 `production` tier, (b) `/api/runtime/health` 가 `ok` 이고 tier=`production`, (c) `panel_truncated_at_limit == false`, (d) `harness-retention-archive` 가 운영 스케줄로 돌고 있음, (e) `brain_bundle_v2_integrity_failed == false`. 상세 evidence 는 `data/mvp/evidence/agentic_operating_harness_v1_milestone_20_patch_9_{bridge,runbook,production_bundle_actualization,bounded_self_serve,scale_closure,surface_polish,tests}_evidence.json` + `data/mvp/evidence/screenshots_patch_9/freeze_*.html` + `sha256_manifest.json`.
- **다음 P0**: (a) **CF-8·D `_horizon_lens_compare` 번들 단위 캐시** (F4 이월 마지막 칸), (b) **CF-9·A `harness-retention-archive` 주기화** (Railway cron 또는 `worker:` 사이드채널), (c) **CF-9·B production bundle graduation 의 Supabase r-branch 자동화** (현재 수동 운영자 트리거), (d) 이월된 **추가 sandbox kinds** (Patch 6 CF-6·1), (e) live evaluator → proposal → apply → spectrum refresh 의 **라이브 세션 녹화**. 쉘·커넥터 확장은 여전히 후순위.
...
**한 방 빌드(Slice A)**: 터미널 복붙 전용 절차는 `docs/operator_closeout/METIS_SLICE_A_COPYPASTE_KR.md`. 요약: `cat <<'EOF' > data/mvp/my_metis_bundle_build.json` 로 설정 파일을 만든 뒤 `build-metis-brain-bundle-from-factor-validation --dry-run` → `ok:true`일 때만 `--dry-run` 제거. 실패 시 응답 JSON의 `hint_ko`·`report` 참고.
