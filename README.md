...
- **Agentic Operating Harness v1 (Stage 2.5)**: Patch 1 (live ingest adapter + retry/backoff), Patch 2 (RegistryUpdateProposalV1 → RegistryDecisionPacketV1 → RegistryPatchAppliedPacketV1 승격 다리), Patch 3 (artifact promotion → spectrum refresh → Today reflection), **Patch 4** (validation → governance evaluator가 challenger-ready `ValidationPromotionEvaluationV1` + `RegistryUpdateProposalV1` 자동 방출), **Patch 5 — Research Ask / bounded sandbox closure**: `layer5_intent_router_v1`(deterministic 7-kind), `ResearchAnswerStructureV1` + `validate_research_structured_v1` 가드레일, `SandboxRequestPacketV1` / `SandboxResultPacketV1` + `sandbox_queue`(only `validation_rerun`, operator-gated), `api_governance_lineage_for_registry_entry`에 `sandbox_followups`, Today object detail에 `sandbox_options_v1` + `research_status_badges_v1`. **LLM은 여전히 registry/bundle에 직접 쓰지 못하며**, 모든 승격 경로는 operator `harness-decide approve` + Patch 3 apply를 통과해야 합니다.
- **아직 데모에 가까운 것(실질)**: `data/mvp/metis_brain_bundle_v0.json`은 시드 스펙트럼과 맞춘 **데모/스모크 번들**이며, `deterministic_kernel:v0`·`stub_feature_set` 등으로 **실제 검증 파이프라인이 자동 생산한 모델 패밀리**를 대체하지는 않습니다. 또한 Patch 5의 **live Supabase evaluator smoke**는 Supabase에 `completed factor_validation_run` 행이 아직 없어 defer 상태이며, 대신 provider 코드 + idempotency 테스트 + runbook evidence로 대체됐습니다 (상세: `data/mvp/evidence/agentic_operating_harness_v1_milestone_16_research_ask_sandbox_bridge_evidence.json`).
- **다음 P0**: (a) Patch 6 — `evidence_refresh` / `residual_review` / `replay_comparison` sandbox kinds 추가 및 실제 `completed factor_validation_run` 기반 live evaluator smoke, (b) research acceptance copy 문장 locale 정합, (c) UI에서 `sandbox_options_v1` · `research_status_badges_v1` 렌더러 (지금은 계약만 출력). 쉘·커넥터 확장은 여전히 후순위.
