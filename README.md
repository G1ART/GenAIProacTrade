...
- **Agentic Operating Harness v1 (Stage 2.5)**: Patch 1 (live ingest adapter + retry/backoff), Patch 2 (RegistryUpdateProposalV1 → RegistryDecisionPacketV1 → RegistryPatchAppliedPacketV1 승격 다리), Patch 3 (artifact promotion → spectrum refresh → Today reflection), Patch 4 (validation → governance evaluator가 challenger-ready `ValidationPromotionEvaluationV1` + `RegistryUpdateProposalV1` 자동 방출), Patch 5 (Research Ask / bounded sandbox closure — `layer5_intent_router_v1` + `ResearchAnswerStructureV1` + `SandboxRequestPacketV1` / `SandboxResultPacketV1` + `sandbox_queue`), Patch 6 (Demo Freeze / Product Surface / Live Evidence Closure — Today 4-block 렌더러 + Research 5-section + Replay governance lineage compact + SVG timeline + `locale_coverage` guardrail + `POST /api/sandbox/enqueue` operator-gated), **Patch 7 — Product hardening / UX depth / scale-readiness**: Patch 6 의 product surface 위에 (1) **A1 IA 2-tier 네비게이션** (primary row = Today/Watchlist/Research/Replay/Ask AI, utility row = Journal/Advanced/Reload bundle 강등, 기능 제거 없음), (2) **A2 Today hero/subhead/body 타이포 토큰 + recent-activity mini-list + consolidated audit `<details>`**, (3) **A3 Research 3-cluster 재그룹 + evidence humanize + 제품 톤 locale 카피 + 모든 문자열 tr() 일원화**, (4) **A4 Replay timeline SVG 3-lane** (governed apply / spectrum refresh / sandbox followup) **+ lineage step count summary + per-step time-delta tooltip**, (5) **A5 tooltip multi-part split (SUB_SEP=' · ')** 로 rail/lineage/plot 전반의 hover 가 다차원 요약으로 업그레이드, (6) **B2 bounded action contract card** (will_do / will_not_do / after_enqueue 3 줄) 을 invoke 버튼 바로 위에, (7) **C2a governance_scan dedupe N+1 hoist** (1회 list_packets + O(1) set 멤버십), (8) **C2b /api/today/spectrum rows_limit + total_rows/truncated** (default 200, cap 1000), (9) **C2c /api/replay/governance-lineage limit cap(500)**, (10) **C2d perf_counter stderr JSON 로그 4곳** (`metis_perf`). **Patch 7 은 demo-theater 패치가 아니라 실제 베타 유저의 UX 깊이 + 스케일 준비도를 손보는 product hardening 패치**. 안전 가드는 모두 유지 (LLM 직접 쓰기 없음, `SANDBOX_KINDS=('validation_rerun',)` 유지, operator gate 유지, fake autonomy 금지).
- **아직 데모에 가까운 것(실질)**: `data/mvp/metis_brain_bundle_v0.json`은 여전히 시드 스펙트럼과 맞춘 **데모/스모크 번들**입니다. Patch 7 의 Scale Readiness Note (`docs/plan/METIS_Scale_Readiness_Note_Patch7_v1.md`) 는 현재 ~200 ticker → S&P 500 확장을 막는 **6 findings** 를 evidence-based 로 기술: F1 factor validation cadence row-by-row insert / F2 governance_scan N+1 (이번 패치에서 fix) / F3 bundle panel-fetch 중복 / F4 Today payload unbounded (이번 패치에서 rows_limit 로 부분 해결) / F5 queue/packet retention 부재 / F6 research structured linear scan. Verdict: **Patch 7 은 readiness work 이고 S&P 500 rollout 자체가 아님**; CF-7·1..4 가 다음 concrete patch 범위. 상세 evidence 는 `data/mvp/evidence/agentic_operating_harness_v1_milestone_18_patch_7_{bridge,runbook,ia,invoke,scale}_evidence.json` + `data/mvp/evidence/screenshots_patch_7/freeze_*.html` + `sha256_manifest.json`.
- **다음 P0**: (a) **CF-7·1 factor validation batch RPC** (F1, 500 ticker throughput 의 가장 큰 수), (b) **CF-7·2 bundle panel-fetch dedupe/cache** (F3), (c) **CF-7·3 research structured asset+horizon 인덱스** (F6), (d) **CF-7·4 packet_store retention + queue counter cache** (F5), (e) 이월된 **추가 sandbox kinds** (Patch 6 CF-6·1), (f) live evaluator → proposal → apply → spectrum refresh 의 **라이브 세션 녹화**. 쉘·커넥터 확장은 여전히 후순위.
