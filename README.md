...
- **Agentic Operating Harness v1 (Stage 2.5)**: Patch 1 (live ingest adapter + retry/backoff), Patch 2 (RegistryUpdateProposalV1 → RegistryDecisionPacketV1 → RegistryPatchAppliedPacketV1 승격 다리), Patch 3 (artifact promotion → spectrum refresh → Today reflection), Patch 4 (validation → governance evaluator가 challenger-ready `ValidationPromotionEvaluationV1` + `RegistryUpdateProposalV1` 자동 방출), Patch 5 — Research Ask / bounded sandbox closure (`layer5_intent_router_v1` deterministic 7-kind, `ResearchAnswerStructureV1` + `validate_research_structured_v1` 가드레일, `SandboxRequestPacketV1` / `SandboxResultPacketV1` + `sandbox_queue`, Today object detail에 `sandbox_options_v1` + `research_status_badges_v1`), **Patch 6 — Demo Freeze / Product Surface / Live Evidence Closure**: Today 4-block 렌더러 (summary rail / primary / decision / evidence) + Research 5-section 렌더러 (`research_structured_v1` + `locale_coverage` 배지) + Replay governance lineage compact + inline SVG timeline plot + 공용 tooltip primitive, `locale_coverage ∈ {dual, ko_only, en_only, degraded}` 계약 + guardrail (silent dual-claim 차단), `POST /api/sandbox/enqueue` (`METIS_HARNESS_UI_INVOKE_ENABLED=1` env-gated, 여전히 worker 자동 실행 없음), `tsr.*` bilingual locale strings + 엔지니어링 용어 primary-UI 누수 정적 스캐너. Supabase probe 로 **R branch 확정** (`factor_validation_runs.status='completed'` 25행) 해 live end-to-end 경로가 도달 가능함을 검증, 실제 write 는 Patch 4/5 runbook 으로 분리해 demo freeze 에서 부작용 없음. **LLM은 여전히 registry/bundle에 직접 쓰지 못하며**, UI invoke 경로도 `SandboxRequestPacketV1` enqueue 까지만 허용 (실제 실행은 `harness-tick --queue sandbox_queue` operator CLI).
- **아직 데모에 가까운 것(실질)**: `data/mvp/metis_brain_bundle_v0.json`은 시드 스펙트럼과 맞춘 **데모/스모크 번들**이며, `deterministic_kernel:v0`·`stub_feature_set` 등으로 **실제 검증 파이프라인이 자동 생산한 모델 패밀리**를 대체하지는 않습니다. Patch 6 Supabase probe 는 R branch (25 completed runs) 를 확정했지만, 실제 `governance_scan` cadence 가 라이브 Supabase 로부터 `ValidationPromotionEvaluationV1` 을 emit → proposal → apply → spectrum refresh 로 이어지는 **end-to-end 라이브 세션** 을 cockpit UI 에 기록하는 작업은 다음 이터레이션으로 이월. 상세 evidence 는 `data/mvp/evidence/agentic_operating_harness_v1_milestone_17_*.json` + `data/mvp/evidence/screenshots/freeze_*.html` + `sha256_manifest.json`.
- **다음 P0**: (a) live evaluator → proposal → apply → spectrum refresh 까지의 **라이브 세션 녹화** (Patch 6 의 R branch 를 실제 UI 세션으로 통과), (b) 이월된 추가 sandbox kinds (`evidence_refresh` / `residual_review` / `replay_comparison`), (c) research ask → one-click enqueue UX polish, (d) 실제 브라우저(playwright/Chromium) 기반 스크린샷 캡처. 쉘·커넥터 확장은 여전히 후순위.
